...
function NotificationCenter() {
  ...
  const markRead = async (notification: Notification, shouldNavigate = false) => {
    try {
      ...
      if (shouldNavigate && notification.link) {
      ...
      <div className="mx-auto max-w-5xl space-y-8">
        ...
        <section className="rounded-[2rem] bg-white p-6 shadow-xl">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-black text-slate-950">Recent notifications</h2>
          ...
          {loading ? (
            ...
            <div className="rounded-2xl bg-slate-50 p-8 text-center text-slate-600">No notifications yet.</div>
          ) : (
            <div className="space-y-4">
              {notifications.map((notification) => (
                <article key={notification.id} className={`rounded-3xl border p-5 ${Number(notification.is_read) === 0 ? "bg-white shadow-md" : "bg-slate-50 opacity-75"}`}>
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Number(notification.is_read) === 0 && (
                        <button onClick={() => markRead(notification)} className="rounded-full bg-slate-200 px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-300">Mark read</button>
                      )}
                      {notification.link && (
                        <button onClick={() => markRead(notification, true)} className="rounded-full bg-slate-950 px-4 py-2 text-sm font-bold text-white hover:bg-blue-700">Open</button>
                      )}
                    </div>
                  </div>
                </article>
