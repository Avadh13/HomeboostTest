...
function MortgageRequest() {
    ...
    fetch(`${API_BASE_URL}/mortgage-services`)
      ...
      .then((data) => {
        ...
        if (!form.service_key && loadedServices[0]?.service_key) {
          setForm((current) => ({ ...current, service_key: loadedServices[0].service_key }));
        }
      })
  ...
  useEffect(() => {
    if (selectedFromUrl) {
      setForm((current) => ({ ...current, service_key: selectedFromUrl }));
    }
  }, [selectedFromUrl]);

  const selectedService = useMemo(
    () => services.find((service) => service.service_key === form.service_key),
    [form.service_key, services],
  );
  ...
  const submitRequest = async (event: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/service-requests`, {
        ...
        body: JSON.stringify({
          ...form,
          partnership_slug: partnershipFromUrl,
          source: token
            ? "employee_portal"
            : partnershipFromUrl
              ? "partnership_landing"
  ...
  return (
    <main className="theme-page min-h-screen text-slate-950">
      ...
      <section className="px-4 py-8 md:px-6 lg:py-10">
        <div className="section-container grid gap-6 lg:grid-cols-[0.82fr_1.18fr]">
          <aside className="theme-panel self-start">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-sky-200">Mortgage support</p>
            <h1 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">Start your mortgage support request.</h1>
            <p className="mt-4 text-sm leading-relaxed text-slate-200 md:text-base">{BRAND.description}</p>

            {partnershipFromUrl && (
              <p className="mt-4 rounded-xl bg-white/10 px-4 py-3 text-sm font-bold text-slate-200">
                Employer portal: /{partnershipFromUrl}
              </p>
            )}
              ...
              {services.map((service) => {
                ...
                return (
                  <button
                    key={service.id}
                    type="button"
                    onClick={() => updateField("service_key", service.service_key)}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <span>
                        ...
                        <span className="mt-1 block text-xs leading-relaxed text-slate-300">{service.description}</span>
                      </span>
                    </div>
                  </button>
                );
          ...
          <form onSubmit={submitRequest} className="premium-card p-5 md:p-7">
            <div className="mb-6 flex flex-col gap-4 border-b border-slate-100 pb-5 md:flex-row md:items-start md:justify-between">
              ...
              {selectedService && (
                <span className="rounded-xl bg-blue-50 px-4 py-2 text-sm font-black text-blue-700">
                  {selectedService.icon} {selectedService.short_title || selectedService.title}
                </span>
              )}
            </div>
            ...
            {success && (
              <div className="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-900">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <Link to={token ? "/employee/messages" : "/login"} className="btn-primary">
                    {success.threadId ? "Open Advisor Chat" : "Open Message Center"}
                  </Link>
                  <Link to={token ? "/resources" : "/contact"} className="btn-secondary">
                    {token ? "Browse Resources" : "Contact HomeBoost"}
                  </Link>
                </div>
              </div>
            )}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="block md:col-span-2">
                <span className="mb-2 block text-sm font-black text-slate-700">Service needed</span>
                <select className="form-field" value={form.service_key} onChange={(event) => updateField("service_key", event.target.value)}>
                  <option value="">Choose service</option>
                  {services.map((service) => <option key={service.id} value={service.service_key}>{service.title}</option>)}
                </select>
              </label>

              <label className="block">
                <span className="mb-2 block text-sm font-black text-slate-700">Full name</span>
                <input className="form-field" value={form.full_name} onChange={(event) => updateField("full_name", event.target.value)} placeholder="Your full name" />
              </label>

              <label className="block">
                <span className="mb-2 block text-sm font-black text-slate-700">Email</span>
                <input className="form-field" type="email" value={form.email} onChange={(event) => updateField("email", event.target.value)} placeholder="you@example.com" />
              </label>

              <label className="block">
                <span className="mb-2 block text-sm font-black text-slate-700">Phone</span>
                <input className="form-field" value={form.phone} onChange={(event) => updateField("phone", event.target.value)} placeholder="(555) 123-4567" />
              </label>

              <label className="block">
                <span className="mb-2 block text-sm font-black text-slate-700">Preferred contact</span>
                <select
                  className="form-field"
                  value={form.preferred_contact_method}
                  onChange={(event) => updateField("preferred_contact_method", event.target.value as RequestForm["preferred_contact_method"])}
                >
              ...
              <label className="block md:col-span-2">
                <span className="mb-2 block text-sm font-black text-slate-700">Preferred contact time</span>
                <input className="form-field" value={form.preferred_time} onChange={(event) => updateField("preferred_time", event.target.value)} placeholder="Example: Weekdays after 5 PM" />
              </label>

              <label className="block md:col-span-2">
                <span className="mb-2 block text-sm font-black text-slate-700">Details</span>
                <textarea className="form-field min-h-[150px]" value={form.message} onChange={(event) => updateField("message", event.target.value)} placeholder="Tell the advisor what you need help with..." />
              </label>
            </div>

            <label className="mt-5 flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm font-semibold text-slate-600">
            ...
            <div className="mt-6 flex flex-wrap justify-end gap-3 border-t border-slate-100 pt-5">
              <Link to={partnershipFromUrl ? `/${partnershipFromUrl}` : "/"} className="btn-secondary">Cancel</Link>
              <button disabled={submitting || loadingServices} className="btn-primary">
                {submitting ? "Submitting..." : "Submit Mortgage Request"}
              </button>
            </div>
          </form>
        </div>
