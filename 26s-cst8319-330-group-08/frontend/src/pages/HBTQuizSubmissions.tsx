...
function HBTQuizSubmissions() {
  ...
  const hiddenOldAttempts = submissions.reduce((sum, item) => sum + Number(item.older_submission_count || 0), 0);
    ...
    <main className="theme-page min-h-screen px-4 py-6 md:px-6 md:py-8">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="theme-panel">
          ...
          <p className="mt-6 text-xs font-black uppercase tracking-[0.25em] text-violet-200">Latest Quiz Follow-Up Center</p>
          <h1 className="mt-3 text-4xl font-black">Latest Employee Quiz Submissions</h1>
          <p className="mt-3 max-w-3xl text-violet-100">Only the latest submission for each employee and quiz appears here. Older attempts are automatically hidden so follow-up stays clean.</p>
          <div className="mt-6 grid gap-4 sm:grid-cols-5">
            <div className="rounded-2xl bg-white/10 p-4"><p className="text-sm text-violet-100">Latest Queue</p><p className="mt-1 text-3xl font-black">{submissions.length}</p></div>
            <div className="rounded-2xl bg-white/10 p-4"><p className="text-sm text-violet-100">Old Hidden</p><p className="mt-1 text-3xl font-black">{hiddenOldAttempts}</p></div>
            <div className="rounded-2xl bg-white/10 p-4"><p className="text-sm text-violet-100">Companies</p><p className="mt-1 text-3xl font-black">{companyOptions.length}</p></div>
            <div className="rounded-2xl bg-white/10 p-4"><p className="text-sm text-violet-100">Assigned Members</p><p className="mt-1 text-3xl font-black">{assignedOptions.length}</p></div>
            <div className="rounded-2xl bg-white/10 p-4"><p className="text-sm text-violet-100">New Leads</p><p className="mt-1 text-3xl font-black">{totalNewLeads}</p></div>
          </div>
        </header>

        <section className="premium-card">
          <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="eyebrow">Search & filters</p><h2 className="mt-1 text-2xl font-black text-slate-950">Filter latest submissions</h2><p className="text-sm text-slate-500">Find the newest follow-up by employee, quiz, company, assigned member, or status.</p></div><button onClick={clearFilters} className="btn-secondary">Clear Filters</button></div>
          ...
          <div className="mt-5 rounded-xl bg-slate-50 p-4 text-sm font-semibold text-slate-700">Showing {filteredSubmissions.length} latest submissions. {hiddenOldAttempts > 0 ? `${hiddenOldAttempts} older attempts are hidden from this queue.` : "No older attempts are hidden."}</div>
        </section>

        <section className="premium-card">
          {loading ? <p className="text-slate-600">Loading submissions...</p> : filteredSubmissions.length === 0 ? <p className="rounded-3xl bg-slate-50 p-8 text-center text-slate-500">No latest submissions match your filters.</p> : <div className="grid gap-4 xl:grid-cols-2">{filteredSubmissions.map((submission) => { const assignment = assignmentByEmail.get(String(submission.employee_email || "").toLowerCase()); return <article key={submission.id} className="rounded-3xl border border-slate-100 bg-slate-50 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-violet-600">{submission.company_name}</p><h3 className="mt-1 text-xl font-black text-slate-950">{submission.employee_name}</h3><p className="text-sm font-semibold text-slate-500">{submission.employee_email}</p></div><div className="flex flex-col items-start gap-2 sm:items-end"><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">Latest</span><span className={`w-fit rounded-full px-3 py-1 text-xs font-black ${getStatusBadge(submission.follow_up_status)}`}>{submission.follow_up_status || "new"}</span></div></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-white p-3"><p className="text-xs font-black uppercase text-slate-400">Quiz</p><p className="mt-1 text-sm font-bold text-slate-700">{submission.quiz_title}</p></div><div className="rounded-2xl bg-white p-3"><p className="text-xs font-black uppercase text-slate-400">Assigned</p><p className="mt-1 text-sm font-bold text-slate-700">{assignment?.member_name || "Unassigned"}</p></div><div className="rounded-2xl bg-white p-3"><p className="text-xs font-black uppercase text-slate-400">Progress</p><p className="mt-1 text-sm font-bold text-slate-700">{assignment?.progress_percent || 0}%</p></div></div><p className="mt-3 text-xs font-bold text-slate-500">Submitted {formatDate(submission.submitted_at)}{Number(submission.older_submission_count || 0) > 0 ? ` · ${submission.older_submission_count} older attempt(s) hidden` : ""}</p><div className="mt-4 flex flex-wrap gap-2"><select className="form-field max-w-[220px]" value={submission.follow_up_status || "new"} onChange={(event) => updateFollowUpStatus(submission.id, event.target.value)}>{FOLLOW_UP_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><button onClick={() => setSelectedSubmission(submission)} className="btn-dark">View Latest Answers</button></div></article>; })}</div>}
        </section>

        {selectedSubmission && <section className="premium-card"><div className="mb-5 flex flex-wrap items-center justify-between gap-4"><div><p className="eyebrow">Latest answers</p><h2 className="mt-1 text-2xl font-black text-slate-950">{selectedSubmission.employee_name}</h2><p className="text-sm text-slate-500">{selectedSubmission.quiz_title}</p></div><button onClick={() => setSelectedSubmission(null)} className="btn-secondary">Close</button></div><div className="grid gap-3">{selectedSubmission.answers?.map((answer, index) => <div key={`${answer.question_text}-${index}`} className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-wide text-slate-400">Question</p><h3 className="mt-1 font-black text-slate-950">{answer.question_text}</h3><p className="mt-2 text-sm leading-relaxed text-slate-700">{answer.answer_text || "No answer"}</p></div>)}{(!selectedSubmission.answers || selectedSubmission.answers.length === 0) && <p className="rounded-2xl bg-slate-50 p-5 text-slate-500">No answers found.</p>}</div></section>}
      </div>
      <ChatWidget />
    </main>
  );
