...
const initialBlocks: BuilderBlock[] = [
  ...
  },
  {
    ...
    description: "Create employer-branded portals and assign each company to a Home Buying Team.",
  ...
  },
  {
    ...
    description: "Create and manage Home Buying Team profiles and administrator accounts.",
    status: "Teams",
    route: "/admin/hbts",
    group: "Business",
    icon: "◈",
  ...
  },
  {
    ...
    description: "Review scoped program activity and download operational reports.",
    status: "Analytics",
    route: "/admin/reports",
    group: "Operations",
    icon: "▥",
...
function AdminBuilder() {
  ...
  const visibleBlocks = useMemo(
    () => (filter === "all" ? blocks : blocks.filter((block) => block.group === filter)),
    [blocks, filter],
  );
      ...
      <div className="space-y-5">
        <section className="theme-panel">
          <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-100">Builder Mode</p>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-relaxed text-blue-50 md:text-base">
                Plan the administration workflow, then open each module to manage website content, employer portals, quizzes, resources, reports, and communication.
              </p>
            </div>

            <div className="grid grid-cols-3 gap-2 rounded-2xl bg-white/10 p-3 backdrop-blur">
              <div><p className="text-2xl font-black">{groupStats.CMS}</p><p className="text-[11px] font-bold uppercase text-blue-100">CMS</p></div>
              <div><p className="text-2xl font-black">{groupStats.Business}</p><p className="text-[11px] font-bold uppercase text-blue-100">Business</p></div>
              <div><p className="text-2xl font-black">{groupStats.Operations}</p><p className="text-[11px] font-bold uppercase text-blue-100">Operations</p></div>
            </div>
          </div>
        </section>
        ...
        <section className="premium-card">
          <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-500">This order is a planning aid only and does not change database order.</p>
            </div>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          {visibleBlocks.map((block) => (
            <div
              ...
              className={`group cursor-grab overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg ${
                draggedId === block.id ? "border-blue-500 opacity-70" : "border-slate-200"
              }`}
            >
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-xl font-black text-blue-700">{block.icon}</div>
                    <div>
                      <p className="text-xs font-black uppercase tracking-[0.14em] text-blue-600">{block.status}</p>
                      <h3 className="mt-1 text-xl font-black text-slate-950">{block.label}</h3>
                    </div>
                  </div>
                  <span className="rounded-lg bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">Drag</span>
                </div>

                <p className="mt-4 min-h-[70px] text-sm leading-relaxed text-slate-600">{block.description}</p>

                <div className="mt-5 flex items-center justify-between gap-3">
                  <span className="rounded-lg bg-slate-50 px-3 py-1 text-xs font-black text-slate-500">{block.group}</span>
        ...
        <section className="premium-card">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {[
              "Create Home Buying Teams",
              "Create employer partnerships",
              "Manage employees and roles",
              "Upload or revoke CSV enrollment",
              ...
              <div key={item} className="rounded-xl bg-slate-50 p-4 text-sm font-black text-slate-700">✓ {item}</div>
            ))}
          </div>
        </section>
      </div>
