...
function AdminPartnerships() {
  ...
  const getPortalLoginUrl = (slug: string) => `${window.location.origin}/${encodeURIComponent(slug)}#login`;
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
          <section className="premium-card">
            ...
            <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
              ...
              <label className="block"><span className="mb-1 block text-xs font-black uppercase tracking-wide text-slate-500">External Website URL</span><input className="form-field" value={form.website} onChange={(e) => setForm({ ...form, website: e.target.value })} /></label>
          ...
          <section className="overflow-hidden rounded-[2rem] border border-slate-100 bg-white shadow-xl">
            <div className="border-b border-slate-100 p-5"><p className="eyebrow">Live preview</p><h2 className="mt-1 text-2xl font-black text-slate-950">Portal Branding Preview</h2><p className="mt-1 text-sm text-slate-500">This preview follows the selected logo and colors.</p></div>
