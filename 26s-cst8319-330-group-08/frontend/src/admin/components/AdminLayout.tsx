...
function AdminLayout({ title, children }: AdminLayoutProps) {
  ...
  const quickRail = [
    ...
    { path: "/admin/mortgage-services", label: "Services", icon: "⌁" },
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f8] text-slate-950">
      ...
      <div className="flex min-h-screen">
        ...
        <aside className={`fixed left-0 top-0 z-50 flex h-screen w-72 transform flex-col border-r border-slate-200 bg-white shadow-xl transition-transform duration-300 lg:left-16 lg:z-30 lg:translate-x-0 lg:shadow-none ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="hidden w-[22rem] shrink-0 lg:block" aria-hidden="true" />
        ...
        <main className="min-h-screen min-w-0 flex-1">
          <div className="border-b border-slate-200 bg-white/90 px-5 py-4 backdrop-blur-xl lg:px-7">
            <div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 md:flex-row md:items-center">
              ...
              <div className="flex flex-wrap gap-2"><Link to="/admin/service-requests" className="rounded-full bg-slate-100 px-3.5 py-2 text-xs font-black text-slate-700 hover:bg-slate-200">Mortgage Requests</Link><Link to="/admin/mortgage-services" className="rounded-full bg-slate-100 px-3.5 py-2 text-xs font-black text-slate-700 hover:bg-slate-200">Services</Link><Link to="/admin/footer" className="rounded-full bg-slate-100 px-3.5 py-2 text-xs font-black text-slate-700 hover:bg-slate-200">Footer Builder</Link><Link to="/admin/partnerships" className="rounded-full bg-gradient-to-r from-blue-600 to-violet-600 px-3.5 py-2 text-xs font-black text-white shadow-md shadow-violet-500/20">New Partnership</Link></div>
            </div>
          </div>
