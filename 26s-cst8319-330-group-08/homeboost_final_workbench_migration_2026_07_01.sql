...
-- =========================================================
-- HomeBoost Final Workbench Migration - 2026-07-01
-- Covers mortgage benefit branding, services, intake requests,
-- footer builder defaults, profile fields, and message-thread linkage.
-- Run once after pulling latest code.
-- =========================================================

-- ---------- Helper: add users profile columns safely ----------
...
SELECT 1, 'HomeBoost Mortgage Benefit', 'HB', 'Mortgage & home buying benefit platform.', 'Modern employer portals, advisor communication, mortgage service intake, resources, appointments, and guided home-buying support in one place.', 'Start Mortgage Request', '/mortgage-request', 'Need mortgage guidance?', 'Choose a service, share your details, and connect with the right advisor through HomeBoost.', 'dark', 'three_column', '© 2026 HomeBoost. All rights reserved.'
WHERE NOT EXISTS (SELECT 1 FROM footer_settings WHERE id = 1);

UPDATE footer_settings
SET brand_name='HomeBoost Mortgage Benefit', tagline='Mortgage & home buying benefit platform.', cta_text='Start Mortgage Request', cta_link='/mortgage-request'
WHERE id=1;
...
SELECT * FROM (
  SELECT 'purchase-home', 'Purchasing a Home', 'Purchase', 'Guidance for first-time buyers, next-home buyers, pre-approval, affordability, and next steps.', '🏡', 'from-blue-500 to-sky-500', 1
  UNION ALL SELECT 'renewal-refinance', 'Renewal / Refinance', 'Renewal', 'Review renewal options, refinance strategy, payment goals, and lender alternatives before signing.', '🔁', 'from-violet-500 to-purple-500', 2
  UNION ALL SELECT 'debt-consolidation', 'Debt Consolidation', 'Debt Help', 'Explore mortgage-based strategies to simplify debt, improve cash flow, and plan responsibly.', '💳', 'from-emerald-500 to-teal-500', 3
  UNION ALL SELECT 'self-employed', 'Self-Employed Mortgage', 'Self-Employed', 'Support for business owners, contractors, and non-traditional income documentation.', '💼', 'from-amber-500 to-orange-500', 4
  UNION ALL SELECT 'separation-divorce', 'Separation / Divorce Mortgage', 'Separation', 'Sensitive guidance for buyouts, refinancing, affordability, and next-home planning after separation.', '🤝', 'from-pink-500 to-rose-500', 5
  UNION ALL SELECT 'not-sure-yet', 'Not sure yet', 'Not Sure', 'Start with a simple conversation and let an advisor help identify the right mortgage path.', '✨', 'from-indigo-500 to-violet-500', 6
) seed
WHERE NOT EXISTS (SELECT 1 FROM mortgage_services);

-- ---------- Mortgage request intake ----------
...
-- ---------- Message thread compatibility columns ----------
...
-- ---------- Homepage CMS copy update ----------
UPDATE page_sections ps
JOIN pages p ON ps.page_id = p.id
SET ps.title='Mortgage guidance built into the employee benefit experience.',
...
WHERE p.slug='home' AND ps.section_key='hero';

-- ---------- Verification ----------
SELECT 'mortgage_services' AS table_name, COUNT(*) AS total FROM mortgage_services
