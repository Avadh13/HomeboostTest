...
const amountCents = () => Number(process.env.HBT_PROGRAM_PRICE_CENTS || 99000);
const currency = () => clean(process.env.HBT_PROGRAM_CURRENCY || "cad", 10).toLowerCase();
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO hbt_registrations (full_name, email, phone, company_name, role_title, website_url, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [fullName, email, clean(req.body.phone, 60) || null, companyName, clean(req.body.role_title, 120) || null, clean(req.body.website_url, 255) || null, clean(req.body.notes, 1500) || null]
    );
    ...
    await pool.query("UPDATE hbt_registrations SET checkout_session_id = ? WHERE id = ?", [session.id, registrationId]);
    await pool.query(
      `INSERT INTO payments (registration_id, provider_session_id, amount_cents, currency, status) VALUES (?, ?, ?, ?, 'pending')`,
      [registrationId, session.id, amountCents(), currency()]
    );

    return res.status(201).json({ status: "success", mode: "stripe", registration_id: registrationId, checkout_url: session.url });
  } catch (error) {
    return res.status(500).json({ status: "error", message: "Failed to start HBT signup", error: error.message });
  }
});
...
router.get("/status/:registrationId", async (req, res) => {
  try {
    ...
    return res.status(500).json({ status: "error", message: "Failed to load registration status", error: error.message });
  }
});
