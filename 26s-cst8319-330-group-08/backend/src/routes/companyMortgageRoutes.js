...
const getCompanyPartnershipIds = async (user) => {
  if (user.partnership_id) return [Number(user.partnership_id)];

  const [rows] = await pool.query(
    `SELECT p.id
     FROM partnerships p
     JOIN employers e ON p.employer_id = e.id
     WHERE e.company_admin_id = ? OR p.company_admin_id = ?`,
    [user.id, user.id]
  );
...
router.get("/summary", protect, async (req, res) => {
  try {
    ...
    const partnershipIds = await getCompanyPartnershipIds(req.user);
    if (partnershipIds.length === 0) {
      return res.json({ status: "success", summary: emptySummary });
    }

    const inClause = buildInClause(partnershipIds);
    ...
    const [[totals]] = await pool.query(
      `SELECT
       ...
       WHERE partnership_id IN (${inClause})`,
      partnershipIds
    );
